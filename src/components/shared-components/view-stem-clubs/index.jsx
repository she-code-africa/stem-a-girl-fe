  ...
  return (
    <section className="w-full relative overflow-hidden bg-[#FFF5FA] py-16 md:py-20">
      ...
      <div className="relative w-[90%] max-w-[1256px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-[50px] md:justify-self-center">
          {stemClubSchools.map((school, i) => (
            <div
              key={i}
              // md:max-w-[50%]
              className={`w-full md:max-w-[445px] bg-white rounded-2xl border-2 px-8 py-8 flex items-center justify-center text-center font-semibold text-xl md:text-2xl ${
                i === stemClubSchools.length - 1 &&
                stemClubSchools.length % 2 !== 0
                  ? "md:col-span-2  md:mx-auto w-full"
                  : ""
